...
#+LaTeX_header:   left=1em,right=1em,top=0pt,bottom=0pt, % Small margins in the box
#+LaTeX_header:   colback=#2,
#+LaTeX_header:   colframe=white, % No border on the box
# LaTeX_header:   enlarge left by=0mm,
# LaTeX_header:   boxsep=5pt,
#+LaTeX_header:   arc=0pt,outer arc=0pt, % Don't round corners
#+LaTeX_header:   }

It looks nice to have the boxes be ~textwidth~. But this means
that the background of my headers will gladly coverup the
sidebar (although their contents won't).
